...
function defineStyles() {
    var styles = ['circle.brushed {' + '    stroke: orange;' + '    stroke-width: 2px;' + '    fill: black;' + '    r: 4px;' + '}', 'path.brushed {' + '    stroke: orange;' + '    stroke-width: 3px;' + '    stroke-opacity: 1;' + '}', 'circle.selected {' + '    stroke: orange;' + '    fill: black;' + '}'],
...
function onResize() {
    ...
    //Apply brush.
    this.measures[this.currentMeasure].brush.on('brushstart', function () {}).on('brush', function () {
        var measure = d3.select(this).datum().measure;
        for (var prop in chart.measures) {
            if (prop !== measure) chart.measures[prop].overlay.call(chart.measures[prop].brush.clear());
        }
...
function paneledOutlierExplorer(element, settings) {
  ...
  Object.defineProperty(chart, 'init', { enumerable: false,
    configurable: true,
    writable: true,
    value: init });
