(function (global, factory) {
    ...
    function defineStyles() {
        ...
        '#paneled-outlier-explorer div.wc-layout.wc-small-multiples#Charts {' + '    width: 75%;' + '    float: right;' + '    padding-top: 10px;' + '}', '#paneled-outlier-explorer div.wc-layout.wc-small-multiples#Charts > div.wc-chart {' + '    padding: 0 1em 0 0;' + '}', '#paneled-outlier-explorer div.wc-layout.wc-small-multiples#Charts > div.wc-chart.expanded {' + '    width: 100%;' + ' }', '#paneled-outlier-explorer div.wc-layout.wc-small-multiples#Charts > div.wc-chart .wc-chart-title {' + '    text-align: left;' + '    font-size: .9em;' + '}', '#paneled-outlier-explorer div.wc-layout.wc-small-multiples#Charts > div.wc-chart .chart-button {' + '    float: right;' + '    cursor: pointer;' + '    border: 1px solid black;' + '    border-radius: 3px;' + '    padding: 0px 3px 1px 3px;' + '    font-size: 75%;' + '    margin-left: 5px;' + '    visibility: hidden;' + '}', '#paneled-outlier-explorer div.wc-layout.wc-small-multiples#Charts > div.wc-chart .chart-button:hover {' + '    background: black;' + '    color: white;' + '}', '#paneled-outlier-explorer div.wc-layout.wc-small-multiples#Charts > div.wc-chart text.no-data {' + '    fill: red;' + '    font-size: 0.8em;' + '}', '#paneled-outlier-explorer .normal-range {' + '    fill: green;' + '    fill-opacity: .05;' + '    stroke: green;' + '    stroke-opacity: 1;' + '}', '#paneled-outlier-explorer .n-inlier {' + '    cursor: help;' + '}', '#paneled-outlier-explorer .n-inlier text {' + '    fill: green;' + '    text-anchor: end;' + '    font-size: 10px;' + '    font-weight: bold;' + '}', '#paneled-outlier-explorer .n-inlier rect {' + '    fill: green;' + '    fill-opacity: .05;' + '    stroke: green;' + '    stroke-opacity: 1;' + '}', '#paneled-outlier-explorer .single-point {' + '    stroke-width: 3;' + '    stroke-opacity: 1;' + '}',

        /***--------------------------------------------------------------------------------------\
          Listing
        \--------------------------------------------------------------------------------------***/

        '#paneled-outlier-explorer div.wc-chart#Listing {' + '    width: 75%;' + '    float: right;' + '    padding-top: 10px;' + '    overflow-x: scroll;' + '}', '#paneled-outlier-explorer div.wc-chart#Listing table {' + '    width: 100%;' + '    display: table;' + '}',
    ...
    function syncSettings(settings) {
        ...
        array_settings.forEach(function (s) {
            if (!(syncedSettings[s] instanceof Array)) syncedSettings[s] = typeof syncedSettings[s] === 'string' ? [syncedSettings[s]] : [];
        });

        //Convert unscheduled_visit_pattern from string to regular expression.
        if (typeof syncedSettings.unscheduled_visit_pattern === 'string' && syncedSettings.unscheduled_visit_pattern !== '') {
    ...
    function syncControlInputs(controlInputs, settings) {
        ...
        if (settings.filters.length > 0) settings.filters.forEach(function (filter) {
            syncedControlInputs.push({
                ...
                description: 'filter',
                multiple: false,
                all: filter.all !== undefined ? filter.all : true,
                start: filter.start !== undefined ? filter.start : null
            });
        });

        //Remove unscheduled visit control if unscheduled visit pattern is unscpecified.
        if (!(settings.unscheduled_visit_regex || settings.unscheduled_visit_values)) controlInputs.splice(controlInputs.map(function (controlInput) {
    ...
    function removeVariables() {
        ...
        this.config.variables = d3.set(d3.merge([[this.config.measure_col], [this.config.measure_order_col], [this.config.id_col], this.config.time_cols.map(function (time_col) {
            return time_col.value_col;
        }), this.config.time_cols.map(function (time_col) {
            return time_col.order_col;
        }), [this.config.value_col], [this.config.unit_col], [this.config.lln_col], [this.config.uln_col], this.config.filters ? this.config.filters.map(function (filter) {
            return filter.value_col ? filter.value_col : filter;
        }) : []])).values().filter(function (variable) {
        ...
        if (this.config.variables.indexOf(this.config.lln_col) < 0 || this.config.variables.indexOf(this.config.uln_col) < 0) {
    ...
    function defineData(data) {
        ...
        this.data.IDs = {
            raw: d3.set(this.data.initial.map(function (d) {
        ...
        this.data.raw = this.data.initial.filter(function (d) {
            return !/^\s*$/.test(d[_this.config.measure_col]) && /^[0-9.]+$/.test(d[_this.config.value_col]);
        });
        this.data.filtered = this.data.raw;
        this.data.IDs.filtered = d3.set(this.data.filtered.map(function (d) {
            return d[_this.config.id_col];
        })).values();
        ...
        if (this.data.raw.length !== this.data.initial.length) console.warn(this.data.initial.length - this.data.raw.length + ' non-numeric observations have been removed from the data.');
    }
    ...
    function captureMeasures() {
        ...
        this.config.allMeasures = this.data.raw[0].hasOwnProperty(this.config.measure_order_col) ? [].concat(toConsumableArray(new Set(this.data.raw.map(function (d) {
            return +d[_this.config.measure_order_col];
        })).values())).sort(function (a, b) {
            return a - b;
        }).map(function (value) {
            return _this.data.raw.find(function (d) {
                return +d[_this.config.measure_order_col] === value;
            }).measure_unit;
        }) // sort measures by measure order
        : [].concat(toConsumableArray(new Set(this.data.raw.map(function (d) {
    ...
    function defineVisitOrder() {
        ...
        this.config.time_cols.forEach(function (time_settings) {
            if (time_settings.type === 'ordinal') {
                ...
                if (time_settings.order_col && _this.data.raw[0].hasOwnProperty(time_settings.order_col)) {
                    //Define a unique set of visits with visit order concatenated.
                    visits = d3.set(_this.data.raw.map(function (d) {
    ...
    function layout() {
        ...
        //Navigation bar.
        var navigationBar = this.container.insert('ul', ':first-child').attr('id', 'navigation-bar');
        var navigationButtons = navigationBar.selectAll('li.navigation').data(['Charts', 'Listing']).enter().append('li').classed('navigation', true).classed('active', function (d) {
            return d === 'Charts';
        }).attr('id', function (d) {
            return d + '-nav';
        }).text(function (d) {
        ...
        this.populationAnnotation = navigationBar.append('li').attr('id', 'population-annotation').html('<span id = "n-participants"></span> of ' + '<span id = "N-participants"></span> participant(s) shown (' + '<span id = "n-N-rate"></span>)');

        //Create controls header.
        var controlsTab = this.container.insert('div', ':first-child').attr('id', 'controls-header').text('Controls');

        //Define all-chart toggle.
        var measureListContainer = this.container.select('#left-side').append('ul').attr('id', 'measure-list-container');
        var measureListHeader = measureListContainer.append('div').attr('id', 'measure-list-header');
        var measureListCheckbox = measureListHeader.append('input').attr({
    ...
    function updatePopulationAnnotation() {
        ...
        this.populationAnnotation.select('#n-N-rate').text(d3.format('%')(this.data.IDs.filtered.length / this.data.IDs.raw.length));
    }
    ...
    function applyFilters(d) {
        ...
        if (d.type === 'subsetter') {
            ...
            this.controls.config.inputs.filter(function (input) {
                return input.type === 'subsetter' && input.value !== 'All' && input.value !== undefined;
    ...
    function customizeControls() {
        ...
        var context = this,
            controls = this.controls.wrap.selectAll('.control-group').classed('hidden', function (d) {
            return _this.config.normal_range_method !== 'Standard Deviation' && /standard deviation/i.test(d.label) || _this.config.normal_range_method !== 'Quantiles' && /quantile/i.test(d.label);
        });
        ...
        normalRangeControl.on('change', function (d) {
            ...
            controls.classed('hidden', function (d) {
                return normal_range_method !== 'Standard Deviation' && /standard deviation/i.test(d.label) || normal_range_method !== 'Quantiles' && /quantile/i.test(d.label);
            });
        });
    }
    ...
    function defineData$1() {
        ...
        this.data.results = this.data.raw.map(function (d) {
    ...
    function m__imize(chart) {
        ...
        if (!chart.wrap.classed('expanded')) {
            ...
            chart.parent.wrap.selectAll('.wc-chart').sort(function (a, b) {
                return a.measure === chart.data.measure ? -1 : b.measure === chart.data.measure ? 1 : chart.config.measures.indexOf(a.measure) - chart.config.measures.indexOf(b.measure);
            });
    ...
    function removeChart() {
        ...
        this.wrap.on('mouseover', function () {
            ...
            var li = d3.select('li.measure-item.' + _this.data.measure.replace(/[^a-z0-9-]/gi, '-'));
    ...
    function classChart() {
        this.wrap.classed(this.data.measure.replace(/[^a-z0-9-]/gi, '-'), true).classed('hidden', this.config.measures.indexOf(this.data.measure) === -1);
    }
    ...
    function removeVisitsWithoutData() {
        ...
        if (!this.config.visits_without_data) {
            this.config.x.domain = this.config.x.domain.filter(function (visit) {
                return d3.set(_this.data.raw.map(function (d) {
    ...
    function defineXsettings() {
        ...
        //Update x-object.
        Object.assign(this.config.x, this.config.time_cols.find(function (time_col) {
    ...
    function deriveStatistics() {
        ...
        if (this.config.normal_range_method === 'LLN-ULN') {
            this.lln = function (d) {
                return d instanceof Object ? +d[_this.config.lln_col] : d3.median(_this.data.raw, function (d) {
                    return +d[_this.config.lln_col];
                });
            };
            this.uln = function (d) {
                return d instanceof Object ? +d[_this.config.uln_col] : d3.median(_this.data.raw, function (d) {
            ...
            this.uln = function (d) {
                return d instanceof Object ? d[_this.config.value_col] - 1 : _this.data.results[_this.data.results.length - 1];
            };
        }
    }
    ...
    function deriveVariables$1() {
        ...
        this.data.raw.forEach(function (d) {
            d.outlier = d[_this.config.value_col] < _this.lln(d) || d[_this.config.value_col] > _this.uln(d);
        });
        this.data.IDs.outliers = d3.set(this.data.raw.filter(function (d) {
            return d.outlier;
        }).map(function (d) {
            return d[_this.config.id_col];
        })).values();
    ...
    function defineDisplayedData() {
        ...
        if (!this.config.unscheduled_visits) this.data.displayed = this.data.displayed.filter(function (d) {
            return !d.unscheduled;
        });

        //Remove inliers.
        if (!this.config.inliers) this.data.displayed = this.data.displayed.filter(function (d) {
            return _this.data.IDs.outliers.indexOf(d[_this.config.id_col]) > -1 || _this.parent.data.IDs.selected.indexOf(d[_this.config.id_col]) > -1;
        });

        //Capture IDs that will actually be displayed.
        this.data.IDs.displayed = d3.set(this.data.displayed.map(function (d) {
            return d[_this.config.id_col];
        })).values().sort();
    }
    ...
    function drawNormalRange() {
        if (this.config.normal_range_method) this.svg.insert('rect', '.line-supergroup').classed('normal-range', true).attr({
    ...
    function annotateInliers() {
        ...
        if (!this.config.inliers) {
            ...
            this.inliersAnnotation.text.attr({
                x: 0,
                dx: -10,
                y: this.plot_height,
                dy: 19
            ...
            //tooltip
            this.inliersAnnotation.title.text(nHidden + ' of ' + n + ' participants (' + d3.format('%')(nHidden / n) + ') with entirely normal results are hidden.\nToggle the "Normal range inliers" checkbox to display these participants.');

            //mosue hover
            this.inliersAnnotation.g.on('mouseover', function () {
    ...
    function attachLines() {
        ...
        this.lines.each(function (d, i) {
            ...
            d.lines = d.values.map(function (di, i) {
                ...
                if (i) {
                    line = {
                        x0: _this.config.x.type === 'linear' ? d.values[i - 1].values.x : _this.x(d.values[i - 1].values.x) + _this.x.rangeBand() / 2,
                        y0: d.values[i - 1].values.y,
                        x1: _this.config.x.type === 'linear' ? di.values.x : _this.x(di.values.x) + _this.x.rangeBand() / 2,
                        y1: di.values.y
                    };
                }
                return line;
    ...
    function highlightChart() {
        ...
        this.lines.classed('brushed', function (d) {
            ...
            if (d.lines.length) {
                ...
                //points
                var x = _this.config.x.type === 'linear' ? d.values[0].values.x : _this.x(d.values[0].values.x) + _this.x.rangeBand() / 2;
    ...
    function brush() {
        ...
        //Apply brush.
        this.package.brush.on('brushstart', function () {
            //Clear previous brush.
            if (_this.parent.brushedChart && _this.parent.brushedMeasure !== _this.data.measure) _this.parent.brushedChart.package.overlay.call(_this.parent.brushedChart.package.brush.clear());
            ...
            if (_this.parent.data.IDs.selected.length > 0) _this.parent.multiples.filter(function (multiple) {
                return _this.parent.data.IDs.selected.filter(function (ID) {
        ...
        if ((this.config.extent[0][0] !== this.package.brush.extent()[0][0] || this.config.extent[0][1] !== this.package.brush.extent()[0][1] || this.config.extent[1][0] !== this.package.brush.extent()[1][0] || this.config.extent[1][1] !== this.package.brush.extent()[1][1]) && this.data.measure === this.parent.brushedMeasure) {
            if (this.config.x.type === 'ordinal') {
                ...
                this.config.extent[1][0] = this.config.extent[1][0] * this.plot_width / this.config.previous_plot_width;
            }
    ...
    function rotateXaxisTickLabels() {
        if (this.config.x.rotate_tick_labels) {
            ...
            ticks.filter(function (d) {
                return ('' + d).length > 10;
            }).style('cursor', 'help').text(function (d) {
                return d.slice(0, 7) + '...';
    ...
    function onResize() {
            ...
            //Draw normal range.
            if (this.filtered_data.length == 0) handleNoData.call(this);else {
                    //Draw normal range.
    ...
    function onDraw$1() {
        ...
        if (this.data.filtered.length) this.table.selectAll('tbody tr').classed('brushed', function (d) {
