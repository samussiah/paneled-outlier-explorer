...
function defineMeasureData() {
    ...
    this.results = this.measure_data.map(function (d) {
...
function removeVisitsWithoutData() {
    ...
    if (!this.config.visits_without_data) {
        this.config.x.domain = this.config.x.domain.filter(function (visit) {
            return d3.set(_this.measure_data.map(function (d) {
...
function deriveStatistics() {
    ...
    if (this.config.normal_range_method === 'LLN-ULN') {
        this.lln = function (d) {
            return d instanceof Object ? +d[_this.config.lln_col] : d3.median(_this.measure_data, function (d) {
                return +d[_this.config.lln_col];
            });
        };
        this.uln = function (d) {
            return d instanceof Object ? +d[_this.config.uln_col] : d3.median(_this.measure_data, function (d) {
        ...
        this.uln = function (d) {
            return d instanceof Object ? d[_this.config.value_col] - 1 : _this.results[_this.results.length - 1];
        };
    }
}
...
function deriveVariables$1() {
    ...
    this.abnormalIDs = d3.set(this.measure_data.filter(function (d) {
        return d[_this.config.value_col] < _this.lln(d) || d[_this.config.value_col] > _this.uln(d);
    }).map(function (d) {
    ...
    this.measure_data.forEach(function (d) {
        ...
        d.abnormal = d[_this.config.value_col] < _this.lln(d) || d[_this.config.value_col] > _this.uln(d);
...
function filterData() {
    ...
    if (!this.config.inliers) this.raw_data = this.raw_data.filter(function (d) {
        return d.abnormalID || d.brushedID;
    });
    if (!this.config.unscheduled_visits) this.raw_data = this.raw_data.filter(function (d) {
...
function brush() {
    ...
    this.package.brush.on('brushstart', function () {}).on('brush', function () {
        ...
        if (_this.parent.data.selectedIDs.length > 0) _this.parent.multiples.forEach(function (multiple) {
            multiple.draw();
        });
