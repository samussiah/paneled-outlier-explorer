...
function applyFilters(d) {
    ...
    if (d.type === 'subsetter') {
        ...
        this.controls.config.inputs.filter(function (input) {
            return input.type === 'subsetter' && input.value !== 'All';
...
function init(data) {
    ...
    this.listing.config.cols = Object.keys(data[0]).filter(function (key) {
        return ['measure_unit', 'unscheduled'].indexOf(key) === -1;
...
function brushMarks() {
    ...
    if (this.parent.data.IDs.selected.length) {
        this.parent.listing.draw(this.parent.data.filtered.filter(function (d) {
