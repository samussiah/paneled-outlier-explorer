...
function defineStyles() {
    var styles = ['.hidden {' + '    display: none !important;' + '}', '#measure-list-container {' + '    width: 19%;' + '    float: left;' + '}', '#measure-list-header {' + '    font-size: 150%;' + '    border-bottom: 1px solid lightgray;' + '    font-weight: lighter;' + '    padding-bottom: 1%;' + '    margin-bottom: 1%;' + '    text-align: right;' + '}', '#measure-list-checkbox {' + '    margin-left: 5px;' + '}', '#measure-list {' + '    list-style-type: none;' + '    font-weight: lighter;' + '}', '.measure-item {' + '}', '.measure-item-container {' + '    text-align: right;' + '}', '.measure-checkbox {' + '    margin-left: 5px;' + '    margin-top: 5px;' + '    float: right;' + '}', 'div.wc-layout.wc-small-multiples {' + '    width: 80%;' + '    float: right;' + '    border-left: 1px solid lightgray;' + '}', 'div.wc-layout.wc-small-multiples > div.wc-chart {' + '    padding-right: 1em;' + '}', 'div.wc-layout.wc-small-multiples > div.wc-chart.full-screen {' + '    z-index: 9999;' + '    width: 100%;' + '    height: 100%;' + '    position: fixed;' + '    top: 0;' + '    left: 0;' + '    background: white;' + ' }', 'div.wc-layout.wc-small-multiples > div.wc-chart .chart-button {' + '    float: right;' + '    cursor: pointer;' + '    border: 1px solid black;' + '    border-radius: 3px;' + '    padding: 0px 4px 1px 3px;' + '    margin-left: 5px;' + '}', 'div.wc-layout.wc-small-multiples > div.wc-chart .chart-button:hover {' + '    background: black;' + '    color: white;' + '}', 'circle.brushed {' + '    stroke: orange;' + '    stroke-width: 2px;' + '    fill: black;' + '    r: 4px;' + '}', 'path.brushed {' + '    stroke: orange;' + '    stroke-width: 3px;' + '    stroke-opacity: 1;' + '}', 'circle.selected {' + '    stroke: orange;' + '    fill: black;' + '}'],
...
function onLayout() {
    ...
    if (d3.select('#measure-list-container').size() === 0) {
        var measureListContainer = d3.select(this.config.element).insert('div', ':first-child').attr('id', 'measure-list-container'),
            ...
            d3.select(chart.config.element).selectAll('.wc-chart').classed('hidden', !checked);
            d3.selectAll('.measure-checkbox').property('checked', checked);
        }),
            measureList = measureListContainer.append('ul').attr('id', 'measure-list'),
            measureItems = measureList.selectAll('li.measure').data(this.config.allMeasures).enter().append('li').classed('measure-item', true).each(function (d) {
            //Append div inside list item.
            var measureItemContainer = d3.select(this).append('div').classed('measure-item-container', true).text(d),
        ...
        measureItems.on('change', function (d) {
            ...
            d3.select(chart.config.element).selectAll('.wc-chart').filter(function (di) {
                return di.measure === d;
            }).classed('hidden', !checked);
            //If any checkbox is unchecked, uncheck measureListCheckbox.
            if (measureItems[0].some(function (measureItem) {
    ...
    this.wrap.select('.wc-chart-title').append('span').classed('delete-chart chart-button', true).html('&#10006;').attr('title', 'Remove chart').on('click', function () {
        //Minimize chart.
        ...
        measureItems.filter(function (d) {
            return d === _this.currentMeasure;
        }).select('input').property('checked', false);
        //If any checkbox is unchecked, uncheck measureListCheckbox.
        if (measureItems[0].some(function (measureItem) {
            return measureItem.getElementsByClassName('measure-checkbox')[0].checked;
        })) d3.select('#measure-list-checkbox').attr('title', 'Display all charts').property('checked', false);
    });
...
function brushMarks(chart, points, lines) {
    ...
    var brushedPoints = points.filter(function (d) {
        ...
        allPoints = d3.select(chart.config.element).selectAll('.point-supergroup g.point circle').classed('brushed selected', false);
    ...
    var brushedLines = lines.filter(function (d, i) {
        ...
        d.lines.forEach(function (line, j) {
            sides.forEach(function (side, k) {
                if (!intersection) {
                    intersection = doLineSegmentsIntersect({ x: line.x0, y: line.y0 }, { x: line.x1, y: line.y1 }, { x: side.x0, y: side.y0 }, { x: side.x1, y: side.y1 });
                }
            });
        });
        return intersection;
        ...
        allLines = d3.select(chart.config.element).selectAll('.line-supergroup g.line path').classed('brushed', false);
...
function brush() {
    ...
    if (this.config.extent[0][0] !== this.package.brush.extent()[0][0] || this.config.extent[0][1] !== this.package.brush.extent()[0][1] || this.config.extent[1][0] !== this.package.brush.extent()[1][0] || this.config.extent[1][1] !== this.package.brush.extent()[1][1]) {
