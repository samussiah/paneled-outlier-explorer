...
function init(data) {
    ...
    if (data.length !== sortedData.length) console.warn(data.length - sortedData.length + ' records without numeric results have been removed from the data.');
...
function onPreprocess() {
    ...
    this.config.y.domain = d3$1.extent(this.raw_data.filter(function (d) {
...
function brushMarks(chart, lines) {
    ...
    if (chart.parent.selectedIDs.length) {
        ...
        chart.parent.listing.draw(chart.parent.data.filter(function (d, i) {
            return i < 25;
        }));
...
function onResize() {
    ...
    this.svg.insert('rect', '.line-supergroup').classed('normal-range', true).attr({ 'x': this.x(this.x_dom[0]) - 1,
