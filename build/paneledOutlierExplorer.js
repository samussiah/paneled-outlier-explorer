(function(global, factory) {
    ...
    \--------------------------------------------------------------------------------------***/
                ...
                '#paneled-outlier-explorer {' +
                    ...
                    '    width: 100%;' +
                    '}',
                '#paneled-outlier-explorer .poe-column {' + '    display: inline-block;' + '}',
                ...
                '#paneled-outlier-explorer .poe-column .poe-header {' +
                    ...
                    '    font-size: 150%;' +
                    '}',
                '#paneled-outlier-explorer .poe-column > * > * {' + '    margin: 10px;' + '}',
                '#paneled-outlier-explorer .poe-hidden {' + '    display: none !important;' + '}',
    ...
    \--------------------------------------------------------------------------------------***/
                ...
                '#paneled-outlier-explorer #poe-all-measures {' +
                    ...
                    '    clear: both;' +
                    '}',
                '#paneled-outlier-explorer #poe-all-measures > * {' + '    float: right;' + '}',
    ...
    \--------------------------------------------------------------------------------------***/
                ...
                '#paneled-outlier-explorer #poe-listing .poe-brushed {' +
                    ...
                    '}',
                '#paneled-outlier-explorer #poe-listing {' + '    overflow-x: scroll;' + '}'
            ],
            style = this.test
        ...
        this.data.filtered = this.data.raw.filter(function(d) {
            ...
            for (var variable in _this.filters) {
                if (filtered === false && !(values.length === 1 && values[0] === 'All'))
                    filtered = _this.filters[variable].indexOf(d[variable]) === -1;
            }
    ...
    \-------------------------------------------------------------------------------------------**/
        ...
        this.containers.controls = this.containers.leftColumn
            .append('div')
            .attr('id', 'poe-controls');

        //Measure controls
        this.containers.measures = this.containers.leftColumn
            .append('div')
            .attr('id', 'poe-measures');
    ...
    function defineSettings() {
        this.settings.merged = Object.assign(
            Object.assign(defaults$1.rendererSettings, defaults$1.webchartsSettings),
    ...
    function syncTimeScale() {
        ...
        var xInput = this.controls.config.inputs.filter(function(input) {
            ...
            time_col = this.parent.paneledOutlierExplorer.settings.time_cols.filter(function(
                time_col
            ) {
                        ...
                        line = {
                            x0:
                                context.config.x.type === 'linear'
                                    ...
                                    : context.x(d.values[i - 1].values.x) +
                                      context.x.rangeBand() / 2,
    ...
    function maintainHighlighting() {
        ...
        if (this.POE.data.selectedIDs.length) {
            var context = this;
            this.package.lines
                .classed('poe-brushed', function(d) {
    ...
    function brushMarks$1() {
        ...
        this.parent.paneledOutlierExplorer.data.selectedIDs = brushedLines.data().map(function(d) {
            return d.id;
        });
        ...
        if (this.parent.paneledOutlierExplorer.data.selectedIDs.length) {
            this.parent.paneledOutlierExplorer.data.filtered.forEach(function(d) {
                d.brushed =
                    _this.parent.paneledOutlierExplorer.data.selectedIDs.indexOf(
    ...
    function charts() {
        this.charts = webcharts.createChart(
            this.containers.charts.node(),
            this.settings,
            this.controls
        );
    ...
    function defineData(data) {
        ...
        this.data.raw.forEach(function(d) {
            ...
            if (d[_this.settings.unit_col])
                d.measure_unit =
                    d[_this.settings.measure_col] + ' (' + d[_this.settings.unit_col] + ')';
            else d.measure_unit = d[_this.settings.measure_col];

            d.brushed = false;
        });

        //Define quantitative, initial, and all measure sets.
        this.data.quantitativeMeasures = d3
            .set(
        ...
        this.data.currentMeasures =
            this.settings.measures && this.settings.measures.length
                ? this.settings.measures.filter(function(measure) {
        ...
        this.data.measures = this.data.currentMeasures.concat(
            this.data.quantitativeMeasures.filter(function(measure) {
    ...
    function xAxisControlLabels() {
        ...
        this.controls.wrap
            ...
            .property('label', function(d) {
                return _this.settings.synced.time_cols
    ...
    function addMeasureList() {
        ...
        this.containers.measureList
            ...
            .data(
                this.data.measures.map(function(measure) {
                    return {
                        measure: measure,
    ...
    function addFilterEventListeners() {
        ...
        this.controls.wrap
            ...
            .on('change', function(d) {
                context.filters[d.value_col] = d3
                    .select(this)
